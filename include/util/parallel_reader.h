...
#ifndef ATP_PARALLEL_READER_H
#define ATP_PARALLEL_READER_H
...
public:
	...
	void reset_parallel_reader(const char *data_path, const char *label_path,
                      size_t thread_cnt, size_t batch_size, size_t dstC, size_t dstH, size_t dstW,
                      preprocessor<value_type> *processor = NULL,
                      size_t _max_cpu_batch_cnt = 4, size_t _max_gpu_batch_cnt = 2,
                      bool in_memory = false,
		...
		// this->stop();
		// printf("before delete q;\n");
	
		// pre_m.lock();

		
		// this->clear_threads();
		// printf("clear_threads done;\n");
		this->reset_max_thread_num(thread_cnt);
		// printf("reset_max_thread_num done\n");
		...
		// printf("this->shuffle = shuffle done\n");
