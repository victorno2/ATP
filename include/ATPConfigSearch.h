...
void ThroughputPeakSearch( base_layer_t<float>* first_layer, base_layer_t<float>* last_layer, 
                ...
                size_t gpu_mem_size, double pcie_bandwidth, 
    ...
    for (int batch_size = start_b; batch_size <= end_b; batch_size += step) {
        p->clear_processors();
        // reader->reset_parallel_reader(train_image_bin, train_label_bin, 4, batch_size, C, H, W, p, 4, 2);
        // ((data_layer_t<float>*)first_layer)->reset_data_nchw(batch_size, C, H, W);
        ((data_layer_t<float>*)first_layer)->ResetBatchSize(batch_size, is_seq);
        // ((data_layer_t<float>*)first_layer)->reset_data_rnn(seqLength, batch_size, inputSize, labelSize);
        ...
        // aaa = apt_net->simulate_trainning(net, batch_size, iter_times);
        // p->clear_processors();
        // ((data_layer_t<float>*)first_layer)->ResetBatchSize(batch_size+2, is_seq);
        // // ((data_layer_t<float>*)first_layer)->reset_data_rnn(seqLength, batch_size+2, inputSize, labelSize);
        // apt_net->set_simulator_trainning(p, reader, net, batch_size+2, first_layer, last_layer, NULL, NULL, NULL);
        // printf("after set_simulator_trainning mem_size = %f\n", BYTE_TO_MB(query_used_mem()));
        // aaa = apt_net->simulate_trainning(net, batch_size+2, iter_times);
        // p->clear_processors();
        // ((data_layer_t<float>*)first_layer)->ResetBatchSize(batch_size+4, is_seq);
        // // ((data_layer_t<float>*)first_layer)->reset_data_rnn(seqLength, batch_size+4, inputSize, labelSize);
        // apt_net->set_simulator_trainning(p, reader, net, batch_size+4, first_layer, last_layer, NULL, NULL, NULL);
        // printf("after set_simulator_trainning mem_size = %f\n", BYTE_TO_MB(query_used_mem()));
        // aaa = apt_net->simulate_trainning(net, batch_size+4, iter_times);
		...
		// printf("after simulate_trainning mem_size = %f\n", BYTE_TO_MB(query_used_mem()));
    ...
    // ((data_layer_t<float>*)first_layer)->reset_data_rnn(seqLength, best_batch_size, inputSize, labelSize);
