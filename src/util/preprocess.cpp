...
template<class value_type>
void mean_subtraction_t<value_type>::reset(size_t N_, size_t C_, size_t H_, size_t W_,
                                        const char *mean_file_path, value_type scale) {
	this->N = N_;
	this->C = C_;
	this->H = H_;
...
template<class value_type>
void preprocessor<value_type>::clear_processors() {
	// printf("before clear processors.size() = %d\n", processors.size());
	while(processors.size() != 0) {
		value_type *tmp = tmps.back();
		cudaFreeHost(tmp);
		tmps.pop_back();
		processors.pop_back();
		// printf("processors.size() = %d  ", processors.size());
		// printf("tmps.size() = %d\n", tmps.size());
	}
}
