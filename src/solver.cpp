...
template<class value_type>
void base_solver_t<value_type>::grad_zero(cublasHandle_t *handle, tensor_t<value_type> *weight_grad, tensor_t<value_type> *bias_grad, bool enable_bias) {
    if (weight_grad == NULL) {
        return;
    }
    value_type zero = 0.0;
    size_t total_params = weight_grad->get_N() * weight_grad->get_C() * weight_grad->get_H() * weight_grad->get_W();
    cublas_scal(handle, total_params, &zero, weight_grad->get_gpu_ptr(), 1);
    if (enable_bias) {
        size_t total_params = bias_grad->get_N() * bias_grad->get_C() * bias_grad->get_H() * bias_grad->get_W();
        cublas_scal(handle, total_params, &zero, bias_grad->get_gpu_ptr(), 1);
    }
}
...
//    update_lr(_iter);
    ...
    // regularization(handle, weight, weight_grad); // 先做一次权重衰减
...
void sgd_solver_t<value_type>::calculate_update(cublasHandle_t *handle, tensor_t<value_type> *p,
    ...
    value_type local_lr = this->lr * (-1) * this->get_grad_alpha();
    // value_type local_lr = this->lr * this->get_grad_alpha();
    value_type one = 1;
    ...
    cublas_axpy(handle, total_params, &local_lr, p_grad->get_gpu_ptr(), 1, p->get_gpu_ptr(), 1);
